...
### High-Level Flow – Phase 2 (ASCII diagram)

```text
Phase 2 Commit:

 Preconditions: Migration finished or policy satisfied

 1) Validate pending-heal safety: old brick NOT authoritative for any pending heals
 2) Remove replace-brick xlator
 3) Disconnect old brick
 4) Attach replacement as AFR child (normal voting child)
 5) Trigger final healing for deltas if needed
 6) Decommission old brick

 After commit: AFR topology identical to original but with new brick
 AFR
  └─ Client-Xlator
     └─ Brick (new)
```
...
### Translator Stack Comparison (ASCII)
...
Brick replacement becomes a transactional, redundancy-preserving
operation suitable for production migrations using this two-phase
approach and the `replace-brick` xlator.

---

**Document History**: updated Feb 24 2026 — wording + diagrams added
# Two-Phase Data-Preserving Brick Replacement Design

**Document Version:** 1.0  
**Date:** February 2026  
**Status:** Design Review  
**Authors:** GlusterFS Architecture Team
...
### High-Level Flow – Phase 1 (ASCII diagram)

```text
Before Prepare:                          After Prepare (Phase 1): replace-brick inserted

         Client                       AFR
           │                           │
           ▼                           ▼
      Client-Xlator  ->            AFR (replica)
           │                          / | \
           ▼                         /  |  \
       Client-0                   C0  C1  C2   (C0,C1,C2 = client xlator stacks)
       (old)                       |   |   |
                                   v   v   v
                                replace-brick (parent of C0 in Phase1)
                                ├─ Client-0 -> Brick (old)
                                └─ Dummy-Xlator -> Dummy (replacement)

Notes: The replace-brick xlator becomes a parent of the existing client stack
for the brick being replaced; it has two children in Phase1: the real brick
and a dummy replacement that receives self-accusing xattrops.
```
